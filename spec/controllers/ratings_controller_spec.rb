...
RSpec.describe RatingsController, type: :controller do
  ...
  describe 'POST #create' do
    ...
    let(:request_params) { { method: :post, action: :create, options: { rating: attributes_for(:rating), book_id: book, user_id: user }, format: :json } }
    let(:request_invalid_params) { { method: :post, action: :create, options: { rating: attributes_for(:rating, :invalid), book_id: book, user_id: user }, format: :json } }
    ...
    context 'with valid attributes' do
      ...
      it 'does not save new rating(second) if user voted earlier' do
        expect { 2.times { do_request(request_params) } }.to change(Rating, :count).by(1)
      end
    end
